...
def test_search_returns_request_id_and_results(client: TestClient) -> None:
    ...
    assert body["index_version"] == "index-v2"
    assert body["model_version"] == "hash-embedding-v2"
    assert body["results"]
...
def test_rejects_oversized_body_before_route() -> None:
    ...
    settings = make_settings(max_body_size_bytes=20)
...
def test_rate_limit_returns_429() -> None:
    ...
    settings = make_settings(per_client_rate_per_minute=1, global_rate_per_minute=100)
    with TestClient(create_app(settings)) as local_client:
...
def test_concurrent_rate_limit_is_enforced() -> None:
    ...
    async def run_requests() -> list[int]:
        settings = make_settings(per_client_rate_per_minute=3, global_rate_per_minute=3)
